async function register(data ) {
  let payloadRegister = dataRegister(data.mcc, data.countryCode, data.currency, data.merchantCountryCode,data.classification, data.clientTimestamp)
...
function configHeaderRegister(payloadRegister,path){
 ...
 function dataRegister (mcc,countryCode,currency,merchantCountryCode, classification, clientTimestamp)  {
     ...
     return {
        ...
        registeredName :`Retest ${mcc} ${clientTimestamp} ${generateRandomString(5)}`,
